...
pub fn generate_stream_url() -> String {
    ...
    format!(
        "wss://stream2{:02}.forexpros.com/echo/{:03x}/{:08x}/websocket",
        //1 + rnd.gen::<u16> ( ) % 280,
        rnd.gen::<u8>() % 100,
        rnd.gen::<u16>() % 0xfff,
        rnd.gen::<u32>()
...
mod tests {
    ...
    pub fn test_new() {
        ...
        assert_eq!(true, *found_info.lock().unwrap());
    }
    ...
    pub fn test_generate_stream_url() {
        ...
        assert_eq!(
            Regex::new(
                r#"wss://stream\d+.forexpros.com/echo/[0-9a-zA-Z]{3}/[0-9a-zA-Z]{8}/websocket"#
            )
            .unwrap()
            .is_match(url.as_str()),
            true,
